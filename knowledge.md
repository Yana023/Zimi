...
初期テキストを「字」にし、入力例を「字」「𰻞𰻞麺」「春夏秋冬」「さち」「𪚥」に変更した。OSごとの字形をそのまま確認できるよう、初期書体を端末の標準フォントへ変更した。初期値は `DEFAULT_STATE` に集約されており、新規表示・設定リセット・保存値やURLパラメータが不正な場合の復帰先として共通利用される。
