...
初期テキストを「字」にし、入力例を「字」「𰻞𰻞麺」「春夏秋冬」「さち」「𱁬」に変更した。`𱁬` は“たいと”として知られるU+3106Cの漢字で、表示可否は端末のフォント対応状況に依存する。OSごとの字形をそのまま確認できるよう、初期書体を端末の標準フォントへ変更した。初期値は `DEFAULT_STATE` に集約されており、新規表示・設定リセット・保存値やURLパラメータが不正な場合の復帰先として共通利用される。
...
補助線の選択肢を「十字」「ライン」「なし」に変更し、初期値を「十字」にした。「ライン」は従来の方眼表示を維持し、「十字」は漢字ドリルを模した8px間隔の丸いドットによる縦横中心線とした。文字単位表示と文章表示の両方で同じ表現を使う。

## 2026-07-11 入力と初期化の修正

HTMLの `maxlength` とJavaScriptの `slice` はUTF-16コード単位で数えるため、サロゲートペアを使う漢字や絵文字が見た目より早く上限へ達していた。ネイティブの文字数制限を外し、`Intl.Segmenter` による書記素単位で最大240文字へ制限する共通処理に変更した。改行は文字数に含めず保持し、空行も一字ずつ表示で1行分の領域を確保する。

「設定を初期化」は入力文字をユーザーデータとして保持し、文字サイズ・字間・書体・表示方法・方向・補助線・左右反転だけを初期値へ戻す。

## 2026-07-11 キーボード操作

通常表示ではEscキーで入力テキストを消去できる。日本語入力の変換候補を閉じる操作で誤消去しないよう `KeyboardEvent.isComposing` が有効な間は無視する。全画面表示中のEscキーはブラウザ標準の全画面終了を優先し、テキストを保持する。

## 2026-07-11 テーマ対応

`prefers-color-scheme` に従ってライト／ダークテーマを自動選択する。背景、パネル、入力部品、プレビュー、文字色、補助線、通知をCSSカスタムプロパティへ集約し、ダークテーマ専用のコントラストへ切り替える。ブラウザUI用の `theme-color` もライト・ダーク別に指定した。印刷時は端末テーマにかかわらず白地・黒字へ戻す。

## 2026-07-11 ユーザー指定フォント

プリセットへ「ユーザー指定」を追加し、フォントファミリー名を直接入力してプレビューへ適用できるようにした。指定名は制御文字を除去して100書記素に制限し、CSS文字列としてエスケープしたうえで端末標準フォントをフォールバックに付ける。指定名は表示状態と一緒に `localStorage` と共有URLへ保存する。

Local Font Access APIの `queryLocalFonts()` が利用できる環境では、ユーザー操作時に権限を要求し、許可された端末フォントのファミリー名を重複排除して検索候補へ入れる。このAPIは対応環境が限定され、フォント列挙には許可が必要なため、非対応・拒否時も直接入力だけで機能する構成にした。フォントデータそのものは読み込み・保存・送信しない。

## 2026-07-11 リポジトリ導線

フッターの作者表記横へGitHubリポジトリへの外部リンクを追加した。モバイル幅でも非表示にせず、ソースコードとライセンスへ常にアクセスできるようにした。
